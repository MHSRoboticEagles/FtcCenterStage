...
public class Path extends ArrayList<Waypoint> {
     ...
     */
    public boolean followPath(DrivetrainSubsystem drivetrain) {
    ...
    public double[] newLoop(double xPos, double yPos, double headingDegrees) {
        ...
        Waypoint targetWaypoint = this.get(currentWaypoint);

        targetX = targetWaypoint.getPose().getX();
        targetY = targetWaypoint.getPose().getY();
        targetHeading = Math.toDegrees(targetWaypoint.getPose().getHeading());
        ...
        if (targetWaypoint instanceof PointTurnWaypoint) {
            PointTurnWaypoint ptWaypoint = (PointTurnWaypoint) targetWaypoint;
            distanceThreshold = ptWaypoint.getPositionBuffer();
            angleThreshold = ptWaypoint.getRotationBuffer();
            driveSpeed = ptWaypoint.getMovementSpeed();
            turnSpeed = ptWaypoint.getTurnSpeed();
        }
        if (targetWaypoint instanceof EndWaypoint) {
            EndWaypoint eWaypoint = (EndWaypoint) targetWaypoint;
            distanceThreshold = eWaypoint.getPositionBuffer();
            angleThreshold = eWaypoint.getRotationBuffer();
            driveSpeed = eWaypoint.getMovementSpeed();
            turnSpeed = eWaypoint.getTurnSpeed();
        }

        double x_Distance = targetX - xPos;
        double y_Distance = targetY - yPos;
        double angle_Distance = this.normalizedHeading(headingDegrees - targetHeading);

        double distance = Math.sqrt(Math.pow(x_Distance, 2) + Math.pow(y_Distance, 2));

        if (distance < distanceThreshold && Math.abs(angle_Distance) < angleThreshold) {
            currentWaypoint++;
            return this.newLoop(xPos, yPos, headingDegrees);
        }
        ...
        double xPower = (x_Distance / distance) * driveSpeed;
        double yPower = (y_Distance / distance) * driveSpeed;
        double turnPower = 0;

        if (Math.abs(angle_Distance) > angleThreshold) {
            turnPower = turnSpeed * Math.signum(angle_Distance);
        }
        return new  double[]{xPower, yPower, turnPower};
    }

    public boolean checkIfPathFinished() {
        return currentWaypoint > this.size();
    }
